...
use global_variables
  ...
  write(10,'(a)') '! fixed_to_dust_size = each of dust grains have fixed temperature which may be same or differnet &
                      &for each dust grain;'
...
' ! Switch to turn on Eley-Rideal and Complex Induced Reaction mechanisms (default=0: desactivated)'
  ...
  write(10,'(a,i0,a)') 'is_grain_MRN = ',is_grain_MRN,' ! 1 = MRN distribution; 0 = WD distribution. (for specific case of WD &
                        &see subroutine GRAIN_DIST(INDEX,DTYPE,A,DNDA))'
  write(10,'(a)') ""
  write(10,'(a)') "!*****************************"
  ...
  write(10,'(a)') ""
  write(10,'(a)') '!************************************'
  write(10,'(a)') '!*      1D and number of grains     *'
  write(10,'(a)') '!************************************'
  write(10,'(a)') ""
  write(10,'(a,i0,a)') 'nb_grains_1D = ',nb_grains_1D,' ! used only in multi grain 1D model. Give number of grains in 1D model. &
                    &if it is 1 then 1D_grain_sizes.in is not read '  
  write(10,'(a)') '                  ! and grain parameters are read from 1D_static.dat file. '
  write(10,'(a)') '                  !in single grain and 0D model it is not needed as number of grains is always read from &
                    &grain_sizes.in file'
  write(10,'(a)') ""
  write(10,'(a)') "!*****************************"
...
if (is_dust_1D ==0 ) then
  ...
  do 
    ...
    if (line.ne.'') then
      read(line,*) grain_radii(i), grain_temp(i),CR_PEAK_GRAIN_TEMP_all(i)
      i = i + 1
    endif
  enddo
  else   
...
if (is_dust_1D.eq.1 ) then
  nb_grains = nb_grains_1D
   
  if (nb_grains_1D == 1) then
    ...
    ! we do not read 1D_grain_sizes.in or grain_sizes.in, values are read from 1D_static.dat file
...
real(double_precision), parameter :: r_min= 5.d-7 , r_max=2.5d-5  ! in cm ....minimum and maximum values of radii in MRN model  !5.d-7 3.11914d-6 arbitrary value
